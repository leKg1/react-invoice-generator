  ...
  const contractRows = smartContractList.map((contract, i) => {
    return (
      <Tr key={i} bg={(props.tokenAddress === contract.smartContractAddress) ? "purple.200" : ""}>
        ...
        <Td><Link to={"/contracts/" + contract.smartContractAddress}>{contract.smartContractAddress}</Link></Td>
        <Td><Link to={"/contracts/" + contract.smartContractAddress}>{web3Lib?web3Lib.utils.fromWei(contract.tokenBalance, "ether"):'loading'} {contract.symbol}</Link></Td>
        <Td><Link to={"/contracts/" + contract.smartContractAddress}>{web3Lib?web3Lib.utils.fromWei(contract.totalSupply, "ether"):'loading'}</Link></Td>
        <Td><Link to={"/contracts/" + contract.smartContractAddress}>{web3Lib?web3Lib.utils.fromWei(contract.stakeOf, "ether"):'loading'} {contract.symbol}</Link></Td>
        <Td><Link to={"/contracts/" + contract.smartContractAddress}>{web3Lib?web3Lib.utils.fromWei(contract.totalStakes, "ether"):'loading'} {contract.symbol}</Link></Td>
      </Tr>
    )
  })
